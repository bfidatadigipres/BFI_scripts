...
def create_contributors(priref, nfa_cat, credit_list, platform):
    ...
    LOGGER.info(
        "============= START document_augmented_streaming_castcred script START ============="
    )
            ...
            for k, v in val.items():
                ...
                if pdata is None:
                    ...
                    if not person_priref:
                        LOGGER.info(
                            "Creation of person record failed. Skipping this record for return attempt."
                        )
                        continue
                    LOGGER.info(
...
def append_activity_type(person_priref, activity_type, existing_types, session):
    ...
    try:
        ...
        LOGGER.warning(
            "append_activity_type(): Unable to append activity_type to Person record\n%s",
            err,
        )
        return False
...
def write_payload(payload, person_priref, session):
    ...
    """
    print(
        f"======= REQUESTS: Sending POST request to People database {person_priref} ===================="
    )
    LOGGER.info(
        "======= REQUESTS: Sending POST request to People database ==================== %s",
        person_priref,
    )
