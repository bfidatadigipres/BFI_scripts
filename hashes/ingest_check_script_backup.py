...
def make_folder_inventory(dpath: str, filepath: str) -> dict[str, list[str]]:
    ...
    local_log(
        filepath, f"Processing files within folder {os.path.relpath(dpath, filepath)}:"
    )
    ...
    for root, _, files in os.walk(dpath):
        for file in files:
            ...
            matching = pygrep(
                os.path.join(filepath, "checksum_folder"), hash_number.upper()
            )
...
def main() -> None:
    ...
    file_list: list[str] = [
        ...
        if os.path.isfile(os.path.join(filepath, x)) and x != "ingest_check.log"
    ]
    dir_list: list[str] = [
        x
        for x in os.listdir(filepath)
    ...
    for file in file_list:
        ...
        if len(matching) == 0:
            ...
            local_log(
                filepath,
                f"No matching checksum found across checksum documentation\n - Local MD5: {hash_number.upper()}",
            )
            ...
            for match in matching:
                ...
                if file.upper() in checksum_line.upper():
                    LOGGER.info(
                        "Checksum full match found in file: %s", checksum_match_path
                    )
                    local_log(
                        filepath,
                        f"Match found in file: {checksum_match_path}\n - Local MD5: {hash_number.upper()}\n - Source MD5: {checksum_line}",
                    )
                    move_path = "ingest_match"
                else:
                    LOGGER.info(
                        "Checksum partial match found in file: %s", checksum_match_path
                    )
                    local_log(
                        filepath,
                        f"Partial match found in file: {checksum_match_path}\n - Local MD5: {hash_number.upper()}\n - Source MD5: {checksum_line}",
                    )
            ...
            for match in matching:
                ...
                if file.upper() in checksum_line.upper():
                    ...
                    local_log(
                        filepath,
                        f"** PARTIAL MATCH: Filename {file} not found in checksum match: {checksum_line}",
                    )
                    match_str.append("partial match")
                LOGGER.info(
                    "Checksum %s found in file: %s", match_str, checksum_match_path
                )
                local_log(
                    filepath,
                    f"{match_str[0].title()} found in file: {checksum_match_path}\n - Local MD5: {hash_number.upper()}\n - Source MD5: {checksum_line}",
                )

            if "full match" in str(match_str):
                local_log(
                    filepath,
                    "Filename matched in a checksum document. Moving to ingest_match/ folder",
                )
                local_log(
                    filepath,
                    "** Please investigate why this file has more than one checksum file matches!\n",
                )
                ...
                local_log(
                    filepath,
                    "Filename not matched in checksum document. Moving to partial_match/ folder",
                )
                local_log(
                    filepath,
                    "** Please investigate why this file has more than one checksum file matches!\n",
                ...
                LOGGER.info(
                    "Checksum match, filename NO match. Moving to ingest_partial/"
                )
    ...
    for directory in dir_list:
        ...
        for key, value in hash_dict.items():
            ...
            for match in value:
                ...
                if file_relpath.lower() in match.lower():
                    ...
                    LOGGER.info(
                        "Relpath not matched: %s - Only filename: %s",
                        file_relpath,
                        fname,
                    )
                    full_match = True
                    ...
                    local_log(
                        filepath,
                        f"** PARTIAL MATCH: Filename {fname} not found in checksum match: {match}",
                    )
                    partial_match = True
        ...
        if len(checksum_file_list) > 1:
            LOGGER.warning(
                "More than one checksum document matched to directory:\n%s",
                ", ".join(checksum_file_list),
            )
            local_log(
                filepath,
                f"** PARTIAL MATCH: More than one checksum document matched to directory contents:\n {', '.join(checksum_file_list)}",
            )
            partial_match = True
        ...
        if no_match is True:
            ...
            local_log(
                filepath,
                f"Please see notes above. Not all files have matching checksums for {dir_relpath}",
            )
            local_log(
                filepath,
            ...
            LOGGER.warning(
                "One or more files in folder does not have clean match: %s", dir_relpath
            )
            LOGGER.info("File moved to: %s", os.path.join(filepath, "ingest_partial"))
            local_log(
                filepath,
                f"Please see notes above. Not all files have cleanly matching checksums for {dir_relpath}",
            )
            local_log(
                filepath,
            ...
            LOGGER.info(
                "All files in folder have checksum and filename matched: %s",
                dir_relpath,
            )
            LOGGER.info("File moved to: %s", os.path.join(filepath, "ingest_match"))
            local_log(
                filepath,
                f"All files have matching checksums and filenames for {dir_relpath}",
            )
