...
def create_dictionary(records: Optional[tuple[dict[str, Any]]]) -> dict[str, str]:
    ...
    for rec in records:
        ...
        try:
            ...
            parent_priref = ""
        parent_rec = adlib.retrieve_record(
            CID_API, "works", f'priref="{parent_priref}"', "1", ["object_number"]
        )[1]
        if not parent_rec:
            continue
        try:
        ...
        loop_dict[f"{parent_ob_num}, {parent_priref}"] = (
            f"{time_code_start}, {time_code_end}"
        )
...
def main():
    ...
    files = [
        x
        for x in os.listdir(WATCH_FOLDER)
        if x.startswith("N_") and x.endswith((".mkv", ".MKV"))
    ]
    if not len(files) > 0:
    ...
    for filename in files:
        ...
        if not os.path.exists(filepath):
            LOGGER.info(
                "Skipping: File %s as it doesn't exist in watch folder", filename
            )
            continue

        local_logger(f"---- New file: {filename} ---------------")
        LOGGER.info("Retrieving Priref, Timecode and Part Of data from CID...")
        cid_data_all = cid_retrieve(
            filename,
            f"(utb.content='{filename}' WHEN utb.fieldname='bfi_replay_filename')",
        )

        if not cid_data_all:
            local_logger(
                f"Problem retrieving data from CID, please check {filename} details.\n"
            )
        ...
        if split == "no_split":
            local_logger(
                "No splitting required for this file. Moving to no_split/ folder"
            )
            ...
            if success:
                ...
                local_logger(
                    "FAIL MKV MOVE: Leaving in place for repeat attempt next script run.\n"
                )
                LOGGER.warning(
                    "%s - Failed to move to no_split/ folder. Retry next pass.",
                    filename,
                )
            continue
        ...
        if not os.path.exists(json_path):
            ...
            local_logger(
                "FAIL: JSON file creation. Leaving MKV file in place for repeat attempt next script run.\n"
            )
            continue
        ...
        if not json_size > 10:
            LOGGER.warning(
                "SKIPPING MOVE: Data not written to JSON file correctly. Deleting JSON file for retry."
            )
            local_logger(
                "FAIL: JSON file creation. Leaving MKV file in place for repeat attempt next script run.\n"
            )
        ...
        local_logger(
            f"JSON file created successfully, MKV and JSON being moved to {split}/ folder:"
        )
        ...
        if success:
            ...
            local_logger(
                f"FAIL MKV MOVE: Manual move of MKV file required to {split}/ folder."
            )
            LOGGER.warning(
                "%s - Failed to move to %s/ folder. Manual move required.",
                filename,
                split,
            )
        ...
        if success2:
            ...
            LOGGER.warning(
                "FAIL JSON MOVE: Failed to move JSON for file to %s/ folder.", split
            )
            local_logger(
                f"FAIL JSON MOVE: Manual move of JSON file required to {split}/ folder.\n"
            )
