...
for secondary Multi Machine Environment in F47 (first was DigiBeta)

Dependencies:
1. LOGS/vt10_submitta.log
2. vt10/submissions.csv
3. vt10/errors.csv
'''
...
f = open(os.path.join(LOGS, 'vt10_submitta.log'), 'w' )
f.write( '=== Processing Items in VT10 selections.csv === ' + dt_string + '\n' )
f.close()
...
for n in tqdm(range(batches_per_iteration), desc='Submitta', leave=False):
    ...
    # Load submissions
    print '* Opening vt10 submissions csv...'
    submissions = {}
    ...
    # Load selections
    print '* Opening vt10 selections csv...'
    ...
    f = open(os.path.join(LOGS, 'vt10_submitta.log', 'a' )
    ...
    for i in tqdm(batch.iterrows(), total=batch_size, desc='Optimising', leave=False):
        ...
        # Track submission
        print '* Writing vt10 submissions to vt10 submissions csv...'
        with open('submissions.csv', 'a') as of:
    ...
    if not batch.successfully_completed:
        ...
        print('Error creating VT10 Workflow job: {}'.format(batch.priref))
