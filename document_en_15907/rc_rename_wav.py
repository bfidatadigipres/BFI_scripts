...
def fname_split(filename: str) -> tuple[str, str, str, str]:
    """
...
def conformance_check(filepath: str) -> str:
    ...
    """
    mediaconch_cmd: list[str] = ["mediaconch", "--force", "-p", WAV_POLICY, filepath]
...
def check_range(range_list: list[str]) -> list[str]:
    """
    ...
    for item in range_list:
        ...
        if os.path.isfile(filepath):
            ...
            LOGGER.info(
                "check_range(): WAV FILE %s missing from %s folder",
                item,
                WAV_ARCHIVE_PATH,
            )
            return None
...
def cid_query(
    database: str, search: str, object_number: str
...
def main():
    ...
    wav_files = [
        f for f in os.listdir(WAV_ARCHIVE_PATH) if f.endswith((".wav", ".WAV"))
    ]
    if len(wav_files) == 0:
    ...
    for file in wav_files:
        ...
        if file.endswith(("01of01.wav", "01of01.WAV")):
            ...
            local_log(
                f"============= NEW WAV FILE FOUND ============= {str(datetime.datetime.now())}"
            )
            ...
            if re.match(".+01of*", file):
                ...
                print(range_list)
                local_log(
                    f"============= NEW WAV FILE GROUP FOUND ============= {str(datetime.datetime.now())}"
                )
                local_log(
                    f"First item in multi-reel group found for processing: {file}"
                )
                files_present = check_range(range_list)
                if files_present is None:
                    local_log(
                        "File(s) missing in sequence, leaving this group until all present.\n"
                    )
                    LOGGER.info(
                        "%s range incomplete, leaving until all parts present.\n", file
                    )
                    continue
                else:
        ...
        if multipart:
            ...
            local_log(
                f"Conformance check results for {file} and all parts:\n{success_check_string}"
            )
        ...
        if singleton and "PASS!" in str(success):
            ...
            if len(cid_data[8]) > 0:
                LOGGER.info(
                    "Making new CID item record for WAV using parent title %s",
                    cid_data[8],
                )
                local_log(
                    f"Creating new CID item record using parent title: {cid_data[8]}"
                )
                wav_data = create_wav_record(
                    cid_data[0], cid_data[8], cid_data[9], cid_data[13]
                )
            elif len(cid_data[1]) > 0:
                LOGGER.info(
                    "Making new CID item record for WAV using grandparent title %s",
                    cid_data[1],
                )
                local_log(
                    f"Creating new CID item record using grandparent title: {cid_data[1]}"
                )
                wav_data = create_wav_record(
                    cid_data[0], cid_data[1], cid_data[2], cid_data[6]
                )
            else:
                local_log(
                    f"Unable to retrieve CID data for: {source_ob_num}. Moving file to failed_rename folder."
                )
                LOGGER.warning(
                    "Title information absent from CID data retrieval, skipping record creation"
                )
            ...
            if wav_data:
                ...
                LOGGER.info(
                    "Creation of new WAV item record successful: %s", wav_data[0]
                )
            else:
                LOGGER.warning("No WAV object number obtained - failed record creation")
                local_log(
                    f"FAILED: Creation of new WAV Item record failed for {file}. Leaving to retry"
                )
                continue
            ...
            if success:
                ...
                LOGGER.warning(
                    "Source item link failed, and must be appended manually:"
                )
                LOGGER.warning(
                    "Source item: %s - New WAV Item record: %s",
                    source_ob_num,
                    wav_data[0],
                )
                local_log(
                    "- WARNING! Source item link failed, and must be appended manually:"
                )
                local_log(
                    f"-          Source item: {source_ob_num} - New WAV Item record: {wav_data[0]}"
                )
            ...
            if success is not None:
                ...
                if move_success:
                    ...
                    LOGGER.warning(
                        "FAILED MOVE: %s did not move to autoingest.\n", success[1]
                    )
                    local_log(
                        f"Failed to move {success[1]} to autoingest - please complete this manually\n"
                    )
            else:
                local_log(
                    f"FAIL: File {file} renaming failed, moving to failed_rename folder"
                )
                local_log(
                    f"FAIL: Please rename '{file}' manually to '{success[1]}' and move to autoingest.\n"
                )
                LOGGER.warning(
                    "FAILED ATTEMPT AT RENAMING: Updated log for manual renaming. Moving to failed_rename folder.\n"
                )
                # Move file to failed_rename/ folder
                fail_path = os.path.join(WAV_ARCHIVE_PATH, "failed_rename", file)
                print(f"Moving {filepath} to {fail_path}")
            ...
            local_log(
                f"FAILED POLICY: {file} - Moving file to failed_rename folder for inspection."
            )
            LOGGER.warning(
                "%s failed Mediaconch policy check, skipping record creation and moving to failed_rename folder",
                file,
            )
        ...
        if multipart and "PASS!" in success_check_string:
            ...
            if len(cid_data[7]) > 0:
                ...
                local_log(
                    f"Unable to retrieve CID data for: {source_ob_num}. Moving files to failed_rename folder."
                )
                LOGGER.warning(
                    "Title information absent from CID data retrieval, skipping record creation"
                )
                ...
                for local_file in range_list:
                    ...
                    local_fail_path = os.path.join(
                        WAV_ARCHIVE_PATH, "failed_rename", local_file
                    )
                    print(f"Moving {local_filepath} to {local_fail_path}")
                    local_log(f"Moving {local_file} to {local_fail_path}")
                    LOGGER.info(
                        "Moving %s to fail path: %s", local_file, local_fail_path
                    )
            ...
            for (
                key,
                val,
            ) in success_check.items():
                if "FAIL!" in str(val):
                    filename = os.path.basename(key)
                    local_fail_path = os.path.join(
                        WAV_ARCHIVE_PATH, "failed_rename", filename
                    )
                    print(f"Moving {key} to {local_fail_path}")
                    local_log(
                        f"FAILED MEDIACONCH POLICY: Moving {filename} to {local_fail_path}"
                    )
                    local_log(
                        f"Please resupply and conformance check this file manually. Object number: {wav_data[0]}"
                    )
            ...
            for local_file in range_list:
                ...
                if success is not None:
                    ...
                    LOGGER.info(
                        "File %s renamed successfully to %s", local_file, success[1]
                    )
                    # Move to autoingest
                    local_log(f"Moving to autoingest - file {success[0]}")
                    move_success = ingest_move(success[0], success[1])
                    ...
                    local_log(
                        f"FAIL: File {local_file} renaming failed, moving to failed_rename folder"
                    )
                    local_log(
                        f"FAIL: Please rename '{local_file}' manually after wav object number {wav_data[0]}) and move to autoingest.\n"
                    )
                    LOGGER.warning(
                        "FAILED ATTEMPT AT RENAMING: Updated log for manual renaming. Moving to failed_rename folder.\n"
                    )
            ...
            # Output failure message
            local_log(
                f"FAILED POLICY: {file} and all parts - moving file to failed_rename folder for inspection."
            )
            LOGGER.warning(
                "%s and all parts failed Mediaconch policy check, skipping record creation and moving to failed_rename folder",
                file,
            )
            # Iterate range_list and move file to failed_rename/ folder
            for local_file in range_list:
...
def create_wav_record(
    gp_priref: str, title: str, title_article: str, title_language: str
) -> tuple[str, str]:
    """
...
def append_source(source_ob_num: str, priref: str, ob_num: str) -> bool:
    """
    ...
    if str(data[5]) == str(source_ob_num):
        print(
            f"Retrieved source_item field from Item record: {data[5]} match {source_ob_num}"
        )
        return True
    else:
...
def rename(file: str, ob_num: str) -> tuple[str, str]:
    """
...
def ingest_move(filepath: str, new_filename: str) -> Optional[str]:
    ...
    try:
        ...
        LOGGER.warning(
            "ingest_move(): Failed to move %s to Autoingest %s", new_filename, err
        )
        return None


def log_pprint(dct: dict[str, str]):
    """
