...
HDLR = logging.FileHandler(
    os.path.join(LOG_PATH, "wav_folder_record_create_rename.log")
)
...
def cid_data_retrieval(ob_num: str) -> list[str]:
    ...
    LOGGER.info(
        "Retrieving CID parent manifestation data using query: %s", parent_search
    )
...
def main():
    ...
    if not utils.check_storage(AUTO_WAV_PATH):
    ...
    dirs = [
        x
        for x in os.listdir(WAV_RENAME_PATH)
        if os.path.isdir(os.path.join(WAV_RENAME_PATH, x))
    ]
    ...
    for key, value in directory_list.items():
        ...
        local_log(
            f"============= NEW WAV FOLDER FOUND {folder} ============= {str(datetime.datetime.now())}"
        )
        ...
        if "FAIL" in mediaconch_assess:
            LOGGER.warning(
                "Skipping: One or more WAV file in folder %s has failed the policy",
                folder,
            )
        ...
        if len(source_ob_num) == 0:
            local_log(
                f"Skipping: Unable to retrieve Source object number from folder: {folder}"
            )
            LOGGER.warning(
                "Skipping: Unable to retrieve source object number from folder: %s",
                folder,
            )
            continue
        print(source_ob_num, part_whole)
        local_log(
            f"Source object number retrieved from folder {folder}: {source_ob_num}"
        )
        ...
        if cid_data[15] not in ("Sound", "Combined", "Mixed", "Combined as Sound"):
            LOGGER.info(
                "Skipping: Supplied CID item record source does not have 'sound_item': 'Sound'"
            )
            local_log(
                f"Skipping: Could not find 'Sound' in sound_item record for source item {source_ob_num}"
            )
            continue
        ...
        if len(cid_data[8]) == 0:
            ...
            local_log(
                f"SKIPPING: Could not find priref for source item {source_ob_num}"
            )
            continue
        # Make CID record with Title of source item
        if len(cid_data[9]) > 0:
            LOGGER.info(
                "Making new CID item record for WAV using source item title %s",
                cid_data[9],
            )
            local_log(
                f"Creating new CID item record using source item title: {cid_data[9]}"
            )
            wav_ob_num, wav_priref = create_wav_record(
                cid_data[0], cid_data[9], cid_data[10], cid_data[14], cid_data[8]
            )
        ...
        # Else use Title of manifestation parent
            ...
            LOGGER.info(
                "Making new CID item record for WAV using manifestation parent title %s",
                cid_data[1],
            )
            local_log(
                f"Creating new CID item record using manifestation parent title: {cid_data[1]}"
            )
            wav_ob_num, wav_priref = create_wav_record(
                cid_data[0], cid_data[1], cid_data[2], cid_data[6], cid_data[8]
            )
        else:
            local_log(
        ...
        if wav_ob_num:
            ...
            local_log(
                f"FAILED: Creation of new WAV Item record failed for {folder}. Leaving to retry"
            )
            continue
        ...
        if not append_success:
            LOGGER.warning(
                "Unable to add QUALITY COMMENTS data to CID Item record <%s>\n<%s>",
                wav_priref,
                qual_comm,
            )
        ...
        if success is not None:
            ...
            if move_success:
                ...
                LOGGER.info(
                    "Moved to automation_dpx/for_tar_wrap/ path: %s.\n", move_success
                )
            else:
                LOGGER.warning(
                ...
                local_log(
                    f"Failed to move {success[1]} to automation_dpx/for_tar_wrap/ - please complete this manually\n"
                )
        else:
            local_log(
                f"FAIL: Folder {folder} renaming failed, moving to failed_rename folder"
            )
            local_log(
                f"FAIL: Please rename '{folder}' manually to '{success[1]}' and move to for_tar_wrap.\n"
            )
            LOGGER.warning(
    ...
    LOGGER.info(
        "================ END WAV folder record creation rename END ================="
    )
