...
def retrieve_people_records() -> tuple[int, list[dict[str, Optional[str]]]]:
    ...
    """
    search: str = (
        '(creation>"2019-01-01" and party.class=PERSON and name=* and not (forename.implied_gender=MALE,FEMALE,UNRESOLVED)) and not use=*'
    )
    fields: list[str] = ["priref", "name", "used_for"]
...
def main():
    ...
    if not utils.check_storage(LOGS):
    ...
    for r in records:
        ...
        for n in names:
            if n is None:
                LOGGER.warning(
                    "No names found fpr person priref: %s\n%s", person_priref, r
                )
                continue
            if "," in n:
        ...
        if "Automatic gender determination" in str(success):
            LOGGER.info(
                '* updated record [%s] "%s" to %s', person_priref, names[0], gender
            )
        else:
            LOGGER.warning(
                '* Failed to update record [%s] "%s" to %s',
                person_priref,
                names[0],
                gender,
            )
...
def update_implied_gender(priref: str, gender: str) -> str:
    ...
    p_edit1: str = (
        f"<Edit><edit.date>{now}</edit.date><edit.notes>Automatic gender determination ({gender}) using ONS baby names</edit.notes>"
    )
    p_edit2: str = (
        f"<edit.name>pip:gender</edit.name><edit.time>{time}</edit.time></Edit>"
    )
