...
HDLR = logging.FileHandler(os.path.join(os.environ['LOG_PATH'], 'black_pearl_move_put_reingest.log'))
...
def move_to_ingest_folder(folderpth, file_list, upload_size):
    ...
    '''
    remove_list = []
    logger.info("move_to_ingest_folder(): Moving QNAP IMAGEN STORAGE files to %s", folderpth)
    ...
    for file in file_list:
        ...
        if folder_size < upload_size:
            ...
            logger.info("move_to_ingest_folder(): Moved QNAP IMAGEN STORAGE file into new Ingest folder: %s", file)
    ...
    for f in remove_list:
        if f in file_list:
            file_list.remove(f)
    logger.info("move_to_ingest_folder(): Revised file list in QNAP IMAGEN STORAGE black_pearl_ingest folder: %s", file_list)
