...
transcode_path1="${IS_VID}${TRANS}"
dump_to="${LOG_PATH}mp4_transcode_make_jpeg_files_is_vid.txt"
...
echo " ========================= SHELL LAUNCH - ISILON VIDEO ========================== $date_FULL" >> "${log_path}"
...
grep '/mnt/' "${dump_to}" | parallel --jobs 6 "sudo ${PYENV} $python_script {}"

echo " ========================= SHELL END - ISILON VIDEO ========================== $date_FULL" >> "${log_path}"
