...
HDLR = logging.FileHandler(
    os.path.join(LOGS, "document_augmented_platform_timed_text.log")
)
...
def main():
    ...
    for key, value in STORAGE.items():
        ...
        if len(folder_list) == 0:
            LOGGER.info(
                "%s timed text record creation script. No folders found.", platform
            )
            continue
        ...
        for fpath in folder_list:
            ...
            for file in file_list:
                ...
                if rename_success is False:
                    LOGGER.warning(
                        "Unable to rename file: %s", os.path.join(fpath, file)
                    )
    ...
    LOGGER.info(
        "== Document augmented streaming platform timed text end =====================\n"
    )
...
def create_new_item_record(
    priref: str, fname: str, record: dict[Any, Any]
) -> dict[Any, Any]:
    """
