...
# =========================================================
# Launcher script for exifdata_clean_up.py
# =========================================================
...
LOG="${LOG_PATH}exifdata_clean_up.log"
...
rm "${CODE_PTH}exifdata_clean_up_list.txt"
touch "${CODE_PTH}exifdata_clean_up_list.txt"
...
find "${CID_MEDIAINFO}" -name "*_EXIF.txt" > "${CODE_PTH}exifdata_clean_up_list.txt"

echo " == Launching GNU parallel to run muliple Python3 scripts for metadata_clean_up == " >> "${LOG}"
grep '/mnt/' "${CODE_PTH}exifdata_clean_up_list.txt" | parallel --jobs 10 "${PYENV311} ${CODE_PTH}exifdata_clean_up.py {}"
