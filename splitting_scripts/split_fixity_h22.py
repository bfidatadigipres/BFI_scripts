...
Process H22 digitisation output:
...
def main():
    ...
    for filepath in files:
        ...
        try:
            ...
            logger.warning("models.py error: %s\t%s\n%s", filepath, id_, err)
            print(err)
            continue
        ...
        logger.info(
            "*** Checking number of Items from carrier modelling: %s", len(c.items)
        )
        if len(c.items) > 1:
            print("Multi-item tape found")
            if not MULTI_ITEMS:
                print("Skipping as not running 'multi' pass")
                logger.info("*** Multi item so skipping.")
                continue
        ...
        for item in c.items:
            ...
            logger.info(
                "%s\t* Item priref is %s and object number is %s",
                filepath,
                item_priref,
                object_number,
            )
            ...
            print(
                f"Launching document_item_h22.py to check if CID item record exists: {object_number}"
            )
            try:
                hits, exists = document_item_h22.already_exists(object_number)
            except Exception as err:
                logger.warning(
                    "%s\tUnable to determine if derived record already exists for\t%s\n%s",
                    filepath,
                    object_number,
                    err,
                )
            ...
            if exists:
                ...
                if hits > 1:
                    logger.info(
                        "%s\t* More than one item record for derived MKV already exists for\t%s. Skipping.",
                        filepath,
                        object_number,
                    )
                    print(
                        f"{c.partwhole[0]} is 1 and more than one CID item record exists - this file split will be skipped."
                    )
                    continue
                try:
                    existing_ob_num = adlib.retrieve_field_name(
                        exists, "object_number"
                    )[0]
                except (IndexError, TypeError, KeyError):
                    logger.info("Unable to get object_number for file checks. Skipping")
                    continue
                logger.info(
                    "%s\t* Item record for derived MKV already exists for\t%s",
                    filepath,
                    existing_ob_num,
                )
                ...
                if c.partwhole[0] != 1:
                    print(
                        f"Checking if first part has already been created or has persisted to DPI: {firstpart_check}"
                    )
                    check_result = check_media_record(firstpart_check)
                    firstpart = False
                    if check_result is True:
                        print(
                            f"First part {firstpart_check} exists in CID, proceeding to check for {check_filename}"
                        )
                        firstpart = True
                    if check_result is None:
                        logger.warning(
                            "Skipping: Unable to retrieve CID Media hit information, try again next pass"
                        )
                        continue
                    print(f"AUTOINGEST CHECK: {AUTOINGEST}")
                    match_path = glob.glob(
                        f"{AUTOINGEST}/**/*/{firstpart_check}", recursive=True
                    )
                    ...
                    if not firstpart:
                        logger.info(
                            "%s\tSkipping: First part has not yet been created, no CID match %s",
                            filepath,
                            firstpart_check,
                        )
                        continue
                    logger.info(
                        "%s\tPart 01of* in group found %s. Checking if part also ingested...",
                        filepath,
                        firstpart_check,
                        check_filename,
                    )
                ...
                if check_result is True:
                    print(
                        f"SKIPPING: Filename {check_filename} matched with persisted CID media record"
                    )
                    logger.warning(
                        "%s\tPart found ingested to DPI: %s.", filepath, check_filename
                    )
                    continue
                if check_result is None:
                    print("SKIPPING: API hit attempt failed. Leaving for another pass.")
                    logger.warning(
                        "%s\tAPI attempt returned None. Leaving for another pass: %s.",
                        filepath,
                        check_filename,
                    )
                    continue
                matched = glob.glob(
                    f"{AUTOINGEST}/**/*/{check_filename}", recursive=True
                )
                if check_filename in str(matched):
                    print(
                        f"SKIPPING: CID item record exists and file found in autoingest: {matched[0]}"
                    )
                    logger.warning(
                        "%s\t* Skipping. Part found already in autoingest: %s.",
                        filepath,
                        check_filename,
                    )
                    continue
                logger.info(
                    "%s\t* Item %s not already created. Clear to continue progress.",
                    filepath,
                    check_filename,
                )
            ...
            if os.path.isfile(of):
                ...
                if segments:
                    logger.warning(
                        "%s\tDestination file already exists for segmented file. Deleting: %s",
                        filepath,
                        of,
                    )
                    print(
                        "Has segments, deleting previous version to allow re-creation"
                    )
                    os.remove(of)
                else:
                    logger.warning(
                        "%s\tDestination file already exists for non-segmented file: %s",
                        filepath,
                        of,
                    )
            ...
            if not item_duration:
                ...
                logger.warning(
                    "%s\t* Item has no duration, skipping this file.", filepath
                )
                continue
            ...
            if segments:
                ...
                if pos == 0:
                    ...
                    logger.info(
                        "%s\t* Item is first on tape, starting at tape head...",
                        filepath,
                    )
                else:
                    # Begin at end of previous item
                    print(
                        "* Item is not first on tape, starting at end of previous item..."
                    )
                    logger.info(
                        "%s\t* Item is not first on tape, starting at end of previous item...",
                        filepath,
                    )
                    ...
                    if a < in_:
                        logger.warning(
                            "%s\t%s\tInvalid video_part data: item begins before end of preceding item\t%s",
                            filepath,
                            id_,
                            in_,
                        )
                        print(
                            "Invalid video part data: item begins before end of preceding item"
                        )
                        # Skip entire carrier
                        break
            ...
            if not segments and len(c.items) == 1:
                ...
                logger.info(
                    "%s\t* Item is only one Item on the tape, no need to segment the file...",
                    filepath,
                )
                in_ = 0
                out = item_duration
            ...
            if in_ > out:
                ...
                logger.warning(
                    "%s\t* Duration of tape %s is less than timecode start %s. Skipping this split.",
                    filepath,
                    out,
                    in_,
                )
            ...
            if not make_segment:
                ...
                try:
                    ...
                    print(err)
                    logger.warning(
                        "%s\tUnable to move and rename file\t%s -> %s\n%s",
                        filepath,
                        f,
                        of,
                        err,
                ...
                print(
                    f"Start fixity segmentation of multi-Item file at {datetime.datetime.now().ctime()}"
                )
                logger.info(
                    "%s\tStarting fixity segmentation of multi-Item file at\t%s",
                    filepath,
                ...
                print(
                    f"Call clipmd5.clipmd5({filepath}, {tcin}, {of}, {tcout}, {additional_args})"
                )
                ...
                if not fixity:
                    # Log FFmpeg error
                    ffmpeg_message = (
                        f"{filepath}\tFFmpeg failed to create Matroska file\t{of}"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            "%s\tDeleted invalid Matroska file\t%s", filepath, of
                        )
                    except Exception as err:
                        print(err)
                        logger.warning(
                            "%s\tFailed to delete invalid Matroska file\t%s\n%s",
                            f,
                            of,
                            err,
                        )
            ...
            if not os.path.isfile(of):
                print(
                    f"* Split file does not exist, do not proceed with CID item record creation: {of}"
                )
                logger.info(
                    "%s\tSplit file does not exist. Moving to next item. %s",
                    filepath,
                    of,
                )
                continue
            ...
            logger.info(
                "%s\tEnd fixity segmentation at %s",
                filepath,
                datetime.datetime.now().ctime(),
            )
            ...
            logger.info(
                "LOG CHECK: document_item.new_or_existing(%s, %s, %s, %s, note=%s)",
                object_number,
                relative_segments,
                file_duration,
                extension,
            ...
            print(
                f"Launching document_item_h22.new({object_number}, {relative_segments}, {file_duration}, {extension}, note={note})"
            )
            ...
            if c.partwhole[1] == 1:
                ...
                try:
                    new_object = document_item_h22.new_or_existing(
                        object_number,
                        relative_segments,
                        file_duration,
                        extension,
                    ...
                    if not new_object:
                        ...
                        document_message = (
                            f"{filepath}\tFailed to document Matroska file in CID: {of}"
                        )
                ...
                logger.info(
                    "%s\tCreated/found CID Item for Matroska file: %s",
                    filepath,
                    object_number,
                )
            ...
            # Multi-part
                ...
                # New object successfully retrieved / created
                logger.info(
                    "%s\tCID Item for Matroska file identified: %s / %s",
                    filepath,
                    object_number,
                    new_object,
                )
                print(
                    f"CID item record created/retrieved and linked to source: {object_number} / {new_object}"
                )
            ...
            logger.info(
                "%s\tPart whole retrieved from models: %s", filepath, c.partwhole
            )
            part = str(c.partwhole[0]).zfill(2)
            whole = str(c.partwhole[1]).zfill(2)
            print(new_object, part, whole)
            logger.info(
                "%s\tPart whole script adjustments: %s of %s", filepath, part, whole
            )
            ext = of.split(".")[-1]
            ...
            if os.path.isfile(dst):
                logger.warning(
                    "%s\tFilename already exists in path, skipping renaming: %s",
                    filepath,
                    dst,
                )
                continue
            ...
            try:
                logger.info(
                    "%s\tRenaming Matroska file with new Item object_number and partWhole: %s --> %s",
                    filepath,
                    of,
                    dst,
                )
                ...
                logger.warning(
                    "%s\tFailed to rename Matroska file with new Item object_number and partWhole\t%s\n%s",
                    filepath,
                    of,
                    err,
                )
