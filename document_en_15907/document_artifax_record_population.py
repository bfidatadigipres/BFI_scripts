...
hdlr = logging.FileHandler(
    os.path.join(LOG_PATH, "document_artifax_record_population.log")
)
...
HEADERS = {"X-API-KEY": f"{os.environ['ARTIFAX_API_KEY']}"}
...
FORMATS = {
    "DVD": ["DVD (Digital Versatile Disc)", "73390"],
...
def move_file(fname: str) -> None:
    ...
    if os.path.exists(fname):
        ...
        try:
            ...
            LOGGER.warning(
                "Unable to move %s to completed folder %s\n%s",
                fname,
                file_completed,
                err,
            )
...
def cid_retrieve(search: str) -> Optional[tuple[str | list, str, str]]:
    ...
    record = adlib.retrieve_record(
        CID_API, "works", search, "0", ["grouping.lref", "title", "edit.name"]
    )[1]
...
def work_quick_check(dct=None) -> tuple[str, str, str, str]:
    ...
    priref = cid_import = ""
    for custom_dct in dct["custom_forms"][0]["custom_form_sections"][0][
...
def work_season_retrieve(fname: str, supplied_season_id: str) -> tuple[str]:
    ...
    with open(fname, "r") as inf:
        ...
        for dct in dcts:
            ...
            for custom_dct in dct["custom_forms"][1]["custom_form_sections"][0][
                "custom_form_elements"
            ]:
                cid_import_date = ""
    ...
    with open(fname, "r") as inf:
        ...
        for dct in dcts:
            ...
            if description.upper() in current_festival.upper():
                ...
                if cid_format_type != "" and "internet" in cid_format_type.lower():
                    try:
                        film_format_name = dct["film_format_name"]
                    except (KeyError, IndexError):
                        film_format_name = ""
                    for key, val in FORMATS.items():
                        if film_format_name != "" and str(film_format_name) in key:
                            manifestation_internet_dct.append(
                                {"format_low_level.lref": val[1]}
                            )
                    try:
                    ...
                    try:
                        ...
                        manifestation_internet_dct.append(
                            {"alternative_number.type": "Artifax work_copy id number"}
                        )
                        manifestation_internet_dct.append(
                            {"alternative_number": work_copy_id}
                        )
                    except (KeyError, IndexError):
                        work_copy_id = ""
                # Extract matches that flagged as 'festival'
                if cid_format_type != "" and "festival" in cid_format_type.lower():
                    try:
                        film_format_name = dct["film_format_name"]
                    except (KeyError, IndexError):
                        film_format_name = ""
                    for key, val in FORMATS.items():
                        if film_format_name != "" and str(film_format_name) in key:
                            manifestation_festival_dct.append(
                                {"format_low_level.lref": val[1]}
                            )
                    try:
                        colour_data = dct["colour"]
                ...
                LOGGER.info(
                    "work_copy_extraction(): SKIPPING as festivals don't align."
                )
...
def work_extraction(season_id, dct=None):
    ...
    if len(certification) > 0:
        manifestation_dct.append(
            {"utb.fieldname": "BBFC and BFI festival certifications"}
        )
...
def season_check(season_num, dct=None):
    ...
    if dct is None:
        dct = []
        LOGGER.warning(
            "season_check(): No dictionary data passed through to season_check()"
        )
    try:
        season_id = dct["season_id"]
    except (KeyError, IndexError):
    ...
    if int(season_num) == int(season_id):
        try:
            ...
            LOGGER.warning(
                "season_check(): Unable to obtain accepted value for season appearance"
            )
            accepted = ""
...
def main():
    ...
    LOGGER.info(
        "========== Python3 start: document_artifax_record_population =========="
    )
    ...
    for file in files:
        ...
        LOGGER.info(
            "== Current festival: %s. Festival grouping: %s",
            current_festival,
            festival_grouping,
        )
        ...
        with open(filepath, "r") as inf:
            ...
            for dct in dcts:
                ...
                print(
                    f"Work ID: {work_id}, Priref: {priref}, CID Import: {cid_import}, Art form: {art_form}"
                )
                ...
                try:
                    ...
                    LOGGER.warning(
                        "Skipping as work_season.json not available for work: %s",
                        work_id,
                    )
                    continue
                if not os.path.exists(work_season_fname):
                    LOGGER.warning(
                        "Skipping as work_season.json not present for work_id %s",
                        work_id,
                    )
                    continue
                ...
                if check_work_season is None:
                    LOGGER.warning(
                        "SKIPPING WORK ID: %s Work Season returned None. Check work season data exists: %s",
                        work_id,
                        work_season_fname,
                    )
                    continue
                ...
                if len(priref) > 0 and "yes" not in advanced_confirm.lower():
                    LOGGER.info(
                        "SKIPPING Work ID: %s is not confirmed for advanced record population",
                        work_id,
                    )
                    continue
                if len(priref) == 0:
                    LOGGER.info(
                        "SKIPPING Work ID: %s has no priref present in data.", work_id
                    )
                    continue

                LOGGER.info(
                    "-------------- STAGE ONE: WORK FOUND FOR PROCESSING --------------"
                )
                LOGGER.info(
                    "Artifax work record found for processing priref %s", priref
                )
                ...
                try:
                    ...
                    LOGGER.info(
                        "CID grouping and title data retrieved for priref %s: %s",
                        priref,
                        cid_title,
                    )
                except Exception as err:
                    LOGGER.warning(
                        "Unable to access grouping for priref %s", priref, err
                    )
                    groupings = []
                    cid_title = ""
                    edit_name = ""
                ...
                update_work = make_man = make_qna = append_new_grouping = push_lock = (
                    False
                )
                ...
                if cid_import_date != "":
                    LOGGER.info(
                        "SKIPPING FURTHER STAGES: CID Import date populated and manifestations created already for Festival: %s",
                        festival_grouping,
                    )
                    continue
                ...
                # If edit_name has 'datadigipres' and grouping in CID does not match festival:
                    ...
                    "datadigipres" in str(edit_name)
                    and festival_grouping not in groupings[0]
                ):
                    if len(groupings) >= 3:
                        if festival_grouping in groupings[2]:
                            LOGGER.info(
                                "SKIPPING FURTHER STAGES: Manifestations created already for Festival: %s",
                                festival_grouping,
                            )
                            continue
                    set_one = True
                # Work record data appended. Make manifestations if import date == 0, otherwise skip.
                    ...
                    "datadigipres" in str(edit_name)
                    and festival_grouping in groupings[0]
                ):
                    set_two = True
                # Work found unedited, begin appending/creation of new whole record
                    ...
                    if festival_grouping not in groupings[0]:
                        LOGGER.info(
                            "SKIPPING FURTHER STAGES: Festival groupings do not match for this record %s and %s",
                            festival_grouping,
                            groupings[0],
                        )
                        continue
                    ...
                    LOGGER.warning(
                        "SKIPPING FURTHER STAGES: Unable to identify how to process Work ID: %s",
                        work_id,
                    )
                    continue
                ...
                if set_zero:
                    LOGGER.info(
                        "----------- SKIPPING STAGE TWO - %s EXISTING WORK NOT TO BE OVERWRITTEN ---------",
                        cid_title,
                    )
                    LOGGER.info(
                        "Existing CID record has a new Festival appearance! Creating new Manifestation"
                    )
                    make_man = True
                ...
                if set_one:
                    LOGGER.info(
                        "----------- SKIPPING STAGE TWO - %s WORK RECORD FROM PREVIOUS FESTIVAL ---------",
                        cid_title,
                    )
                    LOGGER.info(
                        "Completed record found with new Festival appearance! Creating new Manifestation"
                    )
                    make_man = True
                    if "yes" in qanda_confirm.lower():
                        make_qna = True
                ...
                if set_two:
                    LOGGER.info(
                        "----------- SKIPPING STAGE TWO - %s WORK RECORD DATA APPENDED ALREADY ---------",
                        cid_title,
                    )
                    LOGGER.info(
                        "New manifestations will still be created and linked to CID Work %s. Work ID %s",
                        priref,
                        work_id,
                    )
                    LOGGER.warning(
                        "This may result in duplicate Manifestations/Q&A records if date lock push failed previously"
                    )
                    make_man = True
                    if "yes" in qanda_confirm.lower():
                        make_qna = True
                ...
                if set_three:
                    LOGGER.info(
                        "CID Work %s being updated, no date lock and festival groupings match",
                        priref,
                    )
                    update_work = True
                    make_man = True
                ...
                if update_work:
                    LOGGER.info(
                        "-------------- STAGE TWO: APPEND WORK DATA TO CID %s --------------",
                        cid_title,
                    )
                    ...
                    if "datadigipres" in str(edit_data[2]):
                        LOGGER.info(
                            "** Successfully appended additional Artifax data to work record %s",
                            priref,
                        )
                    else:
                        LOGGER.warning(
                            "Work update failed for CID Work %s and Artifax work %s. Exiting this record for retry later",
                            priref,
                            work_id,
                        )
                ...
                if make_man:
                    LOGGER.info(
                        "-------------- STAGE THREE: CREATE MANIFESTATION %s --------------",
                        cid_title,
                    )
                    ...
                    if len(work_copy_fname) > 0:
                        print(
                            f"make_manifestations({work_copy_fname}, {current_festival}, {start_date}, {priref}, {manifestation_dct})"
                        )
                        success_man = make_manifestations(
                            work_copy_fname,
                            current_festival,
                            start_date,
                            priref,
                        ...
                        if success_man == "No results":
                            LOGGER.critical(
                                "Error creating manifestation for %s using work copy: %s",
                                priref,
                                work_copy_fname,
                            )
                            LOGGER.warning(
                                "The work copy downloaded is returning 'No Results'"
                            )
                            continue
                        if success_man:
                            LOGGER.info(
                                "** Manifestations created for CID work %s", priref
                            )
                        else:
                            LOGGER.critical(
                                "Error creating manifestation for %s using work copy: %s",
                            ...
                            LOGGER.warning(
                                "Skipping this record's completion. Script to pick up in next pass"
                            )
                            continue
                    else:
                        LOGGER.info(
                            "Work copy filename was not returned from fetch_work_copy(). Exiting for retry later."
                        )
                        continue
                ...
                if make_qna:
                    LOGGER.info(
                        "-------------- STAGE FOUR: CREATE Q&A %s --------------",
                        cid_title,
                    )
                    success_qna = make_qanda(
                        qanda_date,
                        priref,
                        groupings[0],
                        qna_title_dct,
                    ...
                    if success_qna:
                        LOGGER.info(
                            "Q&A Work and Manifestation records created for Priref %s",
                            priref,
                        )
                    else:
                        LOGGER.critical(
                            "Creation of Q&A Work and Manifestation failed for CID record %s and Artifax work %s",
                            priref,
                            work_id,
                        )
                        continue
                ...
                if append_new_grouping:
                    LOGGER.info(
                        "-------------- STAGE FIVE: APPENDING GROUP TO WORK RECORD %s --------------",
                        cid_title,
                    )
                    LOGGER.info(
                        "Pushing new grouping %s to Work record %s",
                        festival_grouping,
                        priref,
                    )
                    ...
                    if success_group:
                        LOGGER.info(
                            "New Festival grouping appended to CID work %s", priref
                        )
                    else:
                        LOGGER.critical(
                            "Failed to update grouping %s to CID Work record priref %s",
                            festival_grouping,
                            priref,
                        )
                        continue
                ...
                if push_lock:
                    LOGGER.info(
                        "-------------- STAGE FIVE: PUSH DATE LOCK TO ARTIFAX %s --------------",
                        cid_title,
                    )
                    LOGGER.info(
                        "Pushing date lock to Artifax work season form with ID: %s",
                        work_season_id,
                    )
                    ...
                    if push_check:
                        ...
                        LOGGER.critical(
                            "FAILED TO WRITE ARTIFAX LOCK TO WORK ID: %s - Script exiting and manual attention will be required",
                            work_id,
                        )
                        continue
                LOGGER.info(
                    "-------------- ALL STAGES COMPLETED SUCCESSFULLY FOR %s --------------",
                    cid_title,
                )
...
def make_manifestations(
    work_copy_fname: str,
    current_festival: str,
    start_date: str,
    priref: str,
    manifestation_dct=None,
) -> bool:
    """
    ...
    if len(manifestation_internet_dct) > 0:
        ...
        LOGGER.info(
            "make_manifestations(): Creation of Internet manifestation record successful. Priref %s",
            man_int_priref,
        )
    else:
    ...
    if len(manifestation_festival_dct) > 0:
        ...
        man_fest_priref = manifestation_create(
            start_date,
            "festival",
            priref,
            manifestation_dct,
        ...
        LOGGER.info(
            "make_manifestations(): Creation of Festival manifestation record successful. Priref %s",
            man_fest_priref,
        )
    else:
    ...
    LOGGER.info(
        "Failed to create manifestation: \n%s\n\n%s\n\n%s",
        manifestation_dct,
        manifestation_internet_dct,
        manifestation_festival_dct,
    )
...
@tenacity.retry(stop=tenacity.stop_after_attempt(5))
def make_qanda(
    qanda_date, priref, grouping, qna_title_dct=None, manifestation_dct=None
) -> Optional[bool]:
    """
    ...
    if len(qna_priref) > 0:
        ...
        man_qna_priref = manifestation_create(
            qanda_date, "qanda", qna_priref, manifestation_dct, None
        )
        if len(man_qna_priref) > 0:
            print(f"** Q&A Manifestation record created with Priref {man_qna_priref}.")
            LOGGER.info(
                "** Q&A Manifestation record created, priref %s", man_qna_priref
            )
            return True
    ...
    for dictionary in dct:
        ...
        if len(credit_name) > 0:
            if "actor" in credit_type.lower():
                ...
                credit_priref_dct.append(
                    ["1", "Cast", "None", f"{credit_ordering}", f"{name}"]
                )
                continue
            ...
            if "scriptwriter" in credit_type.lower() and nfa_cat.lower() == "fiction":
                ...
                continue
            if (
                "scriptwriter" in credit_type.lower()
                and "non fiction" in nfa_cat.lower()
            ):
                name = firstname_split(credit_name)
                credit_priref_dct.append(
                    [
            ...
            if "production company" in credit_type.lower():
                ...
                LOGGER.warning(
                    "%s - credit_type_name unsupported at present (review may be required)",
                    credit_type,
                )
        else:
...
def manifestation_create(
    start_date, event_type, priref, manifestation_dct=None, manifestation_type_dct=None
):
    """
    ...
    if str(event_type) == "festival":
        ...
        manifestation_southbank_defaults = [
            {"manifestationlevel_type": "FESTIVAL"},
            {"format_high_level": "Video - Digital"},
...
def create_qna_work(qna_date: str, film_priref: str, grouping: str, qna_title_dct=None):
    ...
    application_restriction_date: str = str(
        datetime.date.today() + datetime.timedelta(120)
    )
    application_restriction_date_8yr: str = str(
        datetime.date.today() + datetime.timedelta(2922)
    )
    ...
    work_restricted_defaults = (
        {"application_restriction": "DIGITAL_API_PUBLIC"},
    ...
    try:
        ...
        if record:
            try:
                ...
                LOGGER.info(
                    "create_work(): Work record created with priref %s", qna_priref
                )
            except Exception as err:
        ...
        LOGGER.critical(
            "create_work():Unable to create Work Q&A child record for <%s> \n%s",
            film_priref,
            err,
        )
