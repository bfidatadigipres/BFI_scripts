        ...
        for item in c.items:
            ...
            if exists:
                ...
                if check_filename in str(matched):
                    print(f"SKIPPING: CID item record exists and file found in autoingest: {matched[0]}")
            ...
            if c.partwhole[1] == 1:
                ...
                try:
                    new_object = document_item_h22.new_or_existing(object_number, relative_segments, file_duration, extension, note=note)
                    if not new_object:
            ...
            try:
                logger.info('%s\tRenaming Matroska file with new Item object_number and partWhole: %s --> %s', filepath, of, dst)
                print(f'\t{of} --> {dst}')
                os.rename(of, dst)
            except Exception as err:
