...
class Item:
    ...
    def cousins(self):
        ...
        cousins_priref = [
            int(i["priref"][0])
            for i in r.records
            if int(i["priref"][0]) not in self.siblings()
        ]
...
class Containers:
    ...
    def _cans(self, priref):
        ...
        if can_id[:-1].isnumeric() and can_id[-1].isupper():
            ...
            return [
                f"{can_id[:-1]}{i}{can_id[-1]}" for i in string.ascii_uppercase[:whole]
            ]
...
class Carrier:
    ...
    def _partwhole(self):
        ...
        if "name" in self.identifiers:
            ...
            r = cid_get(
                "carriersfull", q, "carrier_part.number,carrier_part.total_numbers"
            )
            if len(r.records) > 1:
    ...
    def segments(self):
        ...
        if len(self.items) > 1 or self.partwhole[1] > 1:
            missing = [
                i["object_number"][0] for i in self.items if "video_part" not in i
            ]
            if missing:
...
class Content:
    ...
    def __init__(self, priref):
        ...
        self.umids = [
            {i: r[i][0] for i in r if i.endswith("_umid")} for r in self.media
        ]

        self.image_urls = [
            f"{DPI_PATH}{i['imagen.media.largeimage_umid']}" for i in self.umids
        ]

        self.images = [
            Image.open(requests.get(i, stream=True, verify=False).raw)
