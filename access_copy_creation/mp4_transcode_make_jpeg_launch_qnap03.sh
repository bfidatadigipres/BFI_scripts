...
transcode_path1="${QNAP_03}${TRANS}"
dump_to="${LOG_PATH}mp4_transcode_make_jpeg_files_qnap03.txt"
...
echo " ========================= SHELL LAUNCH - QNAP03 ========================== $date_FULL" >> "${log_path}"
...
find "${transcode_path1}" -maxdepth 1 -mindepth 1 -type f -mmin +1 >> "${dump_to}"

echo " == Launching GNU parallel to run muliple Python3 scripts for encoding == " >> "${log_path}"
grep '/mnt/' "${dump_to}" | parallel --jobs 2 "sudo ${PYENV} $python_script {}"

echo " ========================= SHELL END - QNAP03 ========================== $date_FULL" >> "${log_path}"
