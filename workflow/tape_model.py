...
class Tape:
    ...
    def _items(self):
        if self.package_number:
            query = (
                f'parts_reference->current_location.name->name="{self.package_number}"'
            )
        elif self.can_id:
            if self.can_id[-1] in [
                "A",
    ...
    def _works(self):
        ...
        for pr in self.objects:
            ...
            record = adlib.retrieve_record(
                CID_API, "works", query, "100", ["object_number", "priref"]
            )[1]
            if record:
    ...
    def get_package_number(self, object_number):
        ...
        record = adlib.retrieve_record(
            CID_API, "carriersfull", search, "1", ["current_location.barcode"]
        )[1]
        if record:
            package = adlib.retrieve_field_name(record[0], "current_location.barcode")[
                0
            ]
            return str(package)
        else:
    ...
    def content_dates(self):
        ...
        for i in work_prirefs:
            ...
            record = adlib.retrieve_record(
                CID_API, "works", search, "1", ["title_date_start"]
            )[1]
    ...
    def origin(self):
        ...
        for i in self.objects:
            ...
            hits, recs = adlib.retrieve_record(
                CID_API, "items", search, "1", ["video_format", "object_number"]
            )
            if hits is None:
                raise Exception(
                    "CID API could not be reached with Items search:\n{seach}"
                )
