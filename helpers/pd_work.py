...
def genre_retrieval(category_code, description, title):
    ...
    for _ in data:
        if category_code in data["genres"]:
            ...
            print(
                f"{category_code} -- New category not in EPG_genre_map.yaml: {title}"
            )
            with open(os.path.join(GENRE_PTH, "redux_undefined_genres.txt"), "a") as genre_log:
                genre_log.write("\n")
                genre_log.write(
...
if __name__ == "__main__":
    ...
    "title.article_cid","title_cid","title.language_cid","title.type_cid"
        ...
        part_of_reference_result = adlib.retrieve_field_name(record[0], 'part_of_reference.lref')
        search = f'priref="{part_of_reference_result[0]}"'
        hit, work_record = adlib.retrieve_record(os.environ.get("CID_API4"), "works", search, "1")
        if work_record == None:
            work_description_type, work_description, work_description_date = "", "", ""

        else:
            work_description_type = adlib.retrieve_field_name(work_record[0], 'description.type')
            work_description = adlib.retrieve_field_name(work_record[0], 'description')
            if work_description == []:
                work_description = ""
            work_description_date = adlib.retrieve_field_name(work_record[0], 'description.date')
        ...
        with open(row["filepath"], 'r') as file:
            ...
            episode_total = info_json.get("item")[0].get("asset").get("meta").get("episodeTotal")
            episode_number = info_json.get("item")[0].get("asset").get("meta").get("episode")
            series_part_unit_value = info_json.get("item")[0].get("asset").get("related")
            if series_part_unit_value == []:
                series_part_unit_value = None
            else:
                series_part_unit_value = info_json.get("item")[0].get("asset").get("related")[0].get("number")
            asset_title = info_json.get('item')[0].get('asset').get('title')
            if asset_title is None:
                asset_title = ''
            category_codes = []
            try:
                category_code_one = info_json.get("item")[0].get("asset").get("category")[0].get("code")
                category_codes.append(category_code_one)
            except (IndexError, KeyError, TypeError) as err:
                print(err)
            try:
                category_code_two = info_json.get("item")[0].get("asset").get("category")[1].get("code")
            ...
            if category_code == []:
                ...
                genre_outcome = None
                genre_one_priref, genre_two_priref, subject_one_priref, subject_two_priref = "", "", "", ""
            ...
            if genre_outcome is None or genre_outcome == []:
                ...
                genre_one_priref, genre_two_priref, subject_one_priref, subject_two_priref = genre_retrieval(category_code, description, asset_title)
            ...
            if episode_number is None:
                results.update({"part_unit.value": "",
            ...
            "description.type_work": work_description_type[0] if type(work_description_type) == list else  "",
            "description_work": work_description[0] if work_description != "" else "",
            "description_date_work": work_description_date[0] if type(work_description_date) == list else  ""
             }
            if description == '':
                results_two.update({
                    "description.type": "",
            ...
            #print(df_dict[0])
            final = {**results_one, **(df_row_dict), **results_two, **descriptions_results, **results}
            print(f"Final: {final}")
