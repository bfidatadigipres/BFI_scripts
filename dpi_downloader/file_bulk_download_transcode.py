...
def get_media_record_data(priref: str) -> list[dict[str, list[str]]]:
    ...
    if hits is None:
        LOGGER.exception(
            "get_media_record_data: Error with API request with search: %s", search
        )
        return []
    if records is None:
        LOGGER.exception(
    ...
    for row in data:
        ...
        if dtype == "single":
            ...
            if "netflix" in str(bucket) or "amazon" in str(bucket):
                LOGGER.warning(
                    "Filename is a Netflix/Amazon item and will not be downloaded"
                )
...
def create_transcode(
    new_fpath: str, transcode: str, fname: str
...
def send_email_update_bulk(
    email: str, download_fpath: str, files_processed: dict[str, str]
) -> None:
    """
    Update user that their item has been
    downloaded, with path, folder and
