        ...
        for line in paths:
            ...
            if any(dt in line for dt in date_range):
                ...
                if 'stream.mpeg2.ts' in files:
                    ...
                    logger.warning("FOUND: stream.mpeg2.ts found %s, folder will be skipped", line)
                    continue
                ...
                for file in files:
                    if file.endswith('.PROBLEM'):
                        ...
                        os.rename(whole_path, os.path.join(head_path, f"PROBLEM_{tail_path}"))

                # Check if folder now updated problem and skip / else process move
                if "PROBLEM_" in line:
                    print(f"*** SKIPPING {line} as PROBLEM_ in foldername ***")
                    logger.warning("FOUND: PROBLEM_ found in foldername %s, folder will be skipped", line)
                    continue
                else:
                    ...
                    try:
                        ...
                        logger.warning("Unable to make new directory: %s\n %s", new_path, error)
                        continue
                    ...
                    if os.path.exists(os.path.join(new_path, foldername)):
                        ...
                        for d in data:
                            try:
                                ...
                                logger.info("Moving folder: %s to %s", os.path.join(line, d), os.path.join(new_path, foldername))
                            except Exception:
                                print(f"Unable to move folder {line}/{d}, into {new_path}/{foldername}")
                                logger.exception("Unable to move folder %s, into %s", os.path.join(line, d), os.path.join(new_path, foldername))
                                continue
                    ...
                    try:
                        ...
                        logger.exception("Unable to move folder %s, into %s", line, new_path)
                        continue

                # New block to move top level recording.log etc only if move above completes
                pth_split_old = os.path.split(line)[0]
                files = [x for x in os.listdir(pth_split_old) if os.path.isfile(os.path.join(pth_split_old, x))]
