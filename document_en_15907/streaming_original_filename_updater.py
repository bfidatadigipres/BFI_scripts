...
HDLR = logging.FileHandler(
    os.path.join(LOGS, "streaming_original_filename_updater.log")
)
...
GROUPINGS = {"Netflix": "400947, IMP", "Amazon": "401361, MOV"}


def cid_check_items(
    grouping: str, file_type: str, platform: str
) -> list[dict[str, str]]:
    """
    ...
    if not record:
        LOGGER.warning(
            "cid_check_items(): Unable to retrieve any %s groupings from CID item records",
            platform,
        )
        return None
...
def cid_check_filenames(priref: str, platform: str) -> Optional[str, str]:
    ...
    if not record:
        LOGGER.warning(
            "cid_check_filenames(): Unable to find CID digital media record match for platform %s: %s",
            platform,
            priref,
        )
        return None
    try:
        file_name_block = adlib.retrieve_field_name(
            record[0], "digital.acquired_filename"
        )
        LOGGER.info(
            "cid_check_filenames(): Total file names retrieved: %s",
            len(file_name_block),
        )
    ...
    for edits in edit:
        try:
            if f"{platform} automated digital acquired filename" in str(
                edits["edit.notes"][0]["spans"][0]["text"]
            ):
...
def cid_check_media(
    priref: str, original_filename: str, ingest_fname: str
    ...
    fields: list[str] = ["priref", "digital.acquired_filename"]
    record = adlib.retrieve_record(CID_API, "media", search, "1", fields)[1]
    if not record:
        LOGGER.warning(
            "cid_check_media(): Unable to find CID digital media record match: %s",
            priref,
        )
        return None, None
...
def main():
    ...
    LOGGER.info(
        "=== Streaming Platform original filename updates START ==================="
    )
    ...
    for key, value in GROUPINGS.items():
        ...
        for priref in priref_list:
            ...
            if edit_date not in str(date_range):
                LOGGER.info(
                    "Skipping priref %s, out of date range: %s", priref, edit_date
                )
                continue
            LOGGER.info(
                "\n* Record found with edit date in range for processing: %s %s",
                priref,
                edit_date,
            )
            ...
            for fname in digital_filenames:
                if " - Renamed to: " in str(fname):
                    ...
                    mpriref, match = cid_check_media(
                        priref, original_fname, ingest_name
                    )
                    if not mpriref:
                        LOGGER.info(
                            "\tIngest asset not found in CID media records: %s",
                            ingest_name,
                        )
                        continue
                    ...
                    if mpriref and match:
                        LOGGER.info(
                            "\tSKIPPING: Digital acquired filename already added to CID digital media record %s - %s",
                            mpriref,
                            original_fname,
                        )
                        continue
                    if mpriref and not match:
                        LOGGER.info(
                            "\tCID media record found %s - Updating digital.acquired_filename to record %s",
                            mpriref,
                            original_fname,
                        )
                        success = update_cid_media_record(
                            mpriref, original_fname, platform, file_type
                        )
                        if not success:
                            LOGGER.warning(
                                "\tFAILED: Update of original filename to CID media record %s: %s",
                                mpriref,
                                original_fname,
                            )
                            continue
                        LOGGER.info(
                            "\tSUCCESS: CID media record %s updated with original filename: %s",
                            mpriref,
                            original_fname,
                        )
                    if not mpriref:
                        LOGGER.info(
                            "\tSKIPPING: No CID media record created yet for ingesting asset: %s",
                            ingest_name,
                        )
                        continue
                else:
                    LOGGER.info(
                        "\tSKIPPING: Acquired filename, not in scope for update: %s - %s",
                        fname,
                        priref,
                    )

    LOGGER.info(
        "=== Streaming Platform original filename updates END ====================="
    )


def update_cid_media_record(
    priref: str, orig_fname: str, platform: str, file_type: str
) -> Optional[bool]:
    """
    ...
    if "edit.notes" in str(update_rec) and names is True:
        LOGGER.info(
            "Successfully appended %s digital.acquired_filenames to CID media record %s",
            file_type,
            priref,
        )
        return True
