...
#END = '2023-08-02T23:59:00'
TITLE_DATA = ''
#TITLE_DATA = 'title=Robin%20Robin'
...
def get_cat_assets(asset=None):
    ...
    try:
        ...
        ep_title = episode_title.replace(' ', '_')
        title = ep_title.replace('/', '').replace('&', 'and').replace("'", "").replace('(', '').replace(')', '')
    ...
    return(title, cat_id, ep_num, episode_asset_id)
...
def get_series_title(asset=None):
    ...
    try:
        title = asset['title']
        title = title.replace('/', '').replace('&', 'and').replace("'", "").replace('(', '').replace(')', '')
...
def main():
    ...
    any new items placing in programme driven folder structures
    '''
    check_control()
    check_api()
    ...
    for asset in items:
        ...
        print(episode_title, catalogue_id, num)
        if num:
            print("Skipping: Episodes not wanted in this script")
            continue
    ...
    for ep_asset_id, cat_id in asset_dict.items():
        ep_cat_id, title = cat_id.split(',')
        ...
        episode_folder = f'{title.strip()}_{ep_asset_id}'

        # Create path to new episode
        mono_path = os.path.join(STORAGE, episode_folder)
