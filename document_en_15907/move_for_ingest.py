...
INPUT_PATH = os.path.join(
    os.environ["IS_MEDIA"], "donor_files/renamed_ready_for_ingest/"
)
...
HDLR = logging.FileHandler(
    os.path.join(os.environ["IS_MEDIA"], "donor_files/test_folder_move.log")
)
...
def main():
    ...
    for _, dirs, _ in os.walk(INGEST_PATH):
        for foldername in dirs:
            ...
            LOGGER.info(
                "* Current folder is %s - searching for %s", foldername, folder_search
            )
            ...
            ingest_jobs: int = len(
                [
                    x
                    for x in file
                    if folder_search in x
                    and "Moved ingest-ready file to BlackPearl ingest" in x
                ]
            )
            LOGGER.info(
                "* Number of successful ingest jobs from folder %s: %s",
                foldername,
                ingest_jobs,
            )
            ...
            if hits == 0:
                LOGGER.info(
                    "* No CID records created for folder, waiting until next time..."
                )
                continue

            gap: int = ingest_jobs - hits
            LOGGER.info(
                "* Number of successful CID Media records from folder %s: %s",
                foldername,
                hits,
            )
            ...
            if gap == 0:
                ...
                LOGGER.info(
                    "* Number of CID Media records created from folder %s in last 30 mins: %s",
                    foldername,
                    g_hits,
                )
                if g_hits == 0:
                    LOGGER.info(
                        "* Ready to move - gap is zero and no Media records created for folder %s in last 30 mins",
                        foldername,
                    )
                    move_folder = True
                else:
                    LOGGER.info(
                        "* NOT ready to move - gap is zero, but %s Media records created for folder %s in last 30 mins - waiting a while...",
                        g_hits,
                        foldername,
                    )
                    move_folder = False
                ...
                LOGGER.info(
                    "* Number of CID Media records created from folder %s in last 60 mins: %s",
                    foldername,
                    g_hits,
                )
                if g_hits == 0:
                    if gap < 100:
                        LOGGER.info(
                            "* Ready to move - gap is less than 100 (%s) and no new Media records created from folder %s in last 60 mins",
                            gap,
                            foldername,
                        )
                        move_folder = True
                        ...
                        LOGGER.info(
                            "* NOT ready to move - gap is greater than 100 (%s) - investigate gap between ingest jobs and CID Media records",
                            gap,
                        )
                        move_folder = False
                else:
                    LOGGER.info(
                        "* NOT ready to move - gap is less than 100 (%s) - but %s Media records created in last hour - waiting a while...",
                        gap,
                        g_hits,
                    )
                    move_folder = False
    ...
    LOGGER.info(
        "==== Move for ingest script completed at %s ===============",
        str(datetime.datetime.now())[0:19],
    )
