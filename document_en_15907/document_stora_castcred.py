...
def main():
    ...
    for fullpath in file_list:
        ...
        work_priref = cid_work_locator(utc_time, asset_id, title, session)[0]
        if not work_priref:
            LOGGER.info(
        ...
        LOGGER.info("Programme asset_id %s and UTC time %s matched to Work: %s" asset_id, utc_time, priref)
        # Check work for evidence of cast/credit
        process_cast = check_work_has_credits(work_priref)
