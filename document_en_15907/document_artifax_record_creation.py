...
hdlr = logging.FileHandler(
    os.path.join(LOG_PATH, "document_artifax_record_creation.log")
)
...
HEADERS = {"X-API-KEY": os.environ["ARTIFAX_API_KEY"]}
...
def fetch_works(season_code, season_id):
    ...
    """
    params = {"season_id": season_id, "season_programming_status_id": 1}
        ...
        for dct in dcts:
            ...
            if THIS_YEAR in extract or NEXT_YEAR in extract:
                ...
                if check[1:-1] in date_range:
                    ...
                    season_list.append(
                        {"season_id": season_id, "season_code": season_code}
                    )
        return season_list
        ...
        for key in FESTIVALS.keys():
            if season_code in key:
                ...
                if str(key_season_id) == str(season_id):
                    ...
                    logger.info(
                        "festival_check(): CURRENT SEASON %s correct and will be selected for record generation",
                        season_code,
                    )
                    json_works.append({"json_works": work_path})
                else:
                    logger.info(
                        "festival_check(): Skipping season record creation: %s Does not match this season: %s",
                        season_code,
                        key_season_id,
                    )
    return json_works
...
def main():
    ...
    logger.info(
        "========== Python3 start: Fetching Artifax JSON data and creating CID record ======="
    )
    ...
    logger.info(
        "Collected JSON path: %s\nCollected season_list: %s\nCollected current festival list: %s",
        json_path,
        season_list,
        json_works,
    )
        ...
        for val in dictionary.values():
            ...
            with open(val, "r") as inf:
                ...
                if str(dcts).startswith("No results"):
                    logger.warning(
                        "Skipping: File does not contain dictionary: %s", val
                    )
                    continue
                season_list = []
                ...
                logger.info(
                    "=== Processing %s, season_code: %s, season_id: %s ===",
                    season_json,
                    season_code,
                    season_num,
                )
                for dct in dcts:
                    ...
                    print(
                        f"Assessing data from work_id dictionary: {work_id} Title: {title}"
                    )
                    ...
                    if not (
                        "film" in art_form.lower() or "television" in art_form.lower()
                    ):
                        logger.info(
                            "Skipping this work as it is not Film or Television: %s",
                            title,
                        )
                    ...
                    print(accepted)
                    if (
                        "Invited - Accepted" in accepted
                        and "xr" not in accepted.lower()
                    ):
                        logger.info("** Title: %s to be processed", title)
                        print(f"Processing: {work_id} Title: {title}")
                        logger.info(
                            "Season performance confirmed for %s - %s. Okay to proceed making new Work record",
                            title,
                            work_id,
                        )
                        ...
                        try:
                            ...
                            logger.info(
                                "No second title to enable country code retrieval"
                            )
                        ...
                        if len(title_data2[0]) > 0:
                            logger.info(
                                "Title 2 present: %s. Checking if it is not English",
                                title_data2[0],
                            )
                            if language2.lower() != "en":
                                # Make title2 main title where foreign language
                                logger.info(
                                    "Title 2 language is not English, writing as main title"
                                )
                                work_data_dct.append({"title": f"{title_data2[0]}"})
                                if len(title_data2[1]) > 0:
                                    work_data_dct.append(
                                        {"title.article": title_data2[1]}
                                    )
                                    print(title_data2[1])
                                else:
                        ...
                        work_data_dct.append(
                            {"alternative_number.type": "Artifax work_id"}
                        )

                        # Obtain season_code priref
                        logger.info(
                            "Obtaining Grouping priref from Festivals dictionary"
                        )
                        for key in FESTIVALS.keys():
                        ...
                        if confirm_priref:
                            logger.info(
                                "Priref successfully written to Artifax: %s", cid_priref
                            )
                        else:
                        ...
                        if confirm_obnum is True:
                            logger.info(
                                "Object number successfully written to Artifax: %s",
                                cid_object_number,
                            )
                        else:
                            logger.warning(
                                "Artifax push confirmation not received for %s",
                                cid_object_number,
                            )
...
def work_extraction(season_num: str, dct=None) -> tuple[str]:
    ...
    if dct is None:
        dct = []
        logger.warning(
            "work_extraction(): No dictionary data passed through to work_extraction()"
        )
    ...
    if not title:
        try:
            ...
            logger.warning(
                "work_extraction():Unable to extract title and language code"
            )
    try:
    ...
    for dictionary in seasons_dict:
        ...
        if int(season_num) == int(season_id):
            try:
                ...
                logger.warning(
                    "work_extraction(): Unable to obtain accepted value for season appearance"
                )
        else:
            logger.info(
                "work_extraction(): Skipping season_id %s does not match this season: %s",
                season_id,
                season_num,
            )
...
def create_work(work_data_dct=None) -> tuple[str, str]:
    ...
    application_restriction_date_8yr = str(
    ...
    try:
        ...
        if record:
            ...
            print(
                f"* Work record created with Priref {cid_priref} Object number {cid_object_number}"
            )
            logger.info("create_work(): Work record created with priref %s", cid_priref)
        else:
            logger.warning(
                "CID priref/object_number is not present after creating CID record"
            )
