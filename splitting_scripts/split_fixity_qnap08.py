...
def main():
    ...
    for root, _, filenames in os.walk(TARGET):
        for filename in [
            f
            for f in filenames
            if f.endswith((".mov", ".mxf", ".mkv", ".MOV", ".MXF", ".MKV"))
        ]:
    ...
    for filepath in files:
        ...
        for item in c.items:
            ...
            print(
                f"Launching document_item.py to check if CID item record exists: {object_number}"
            )
            try:
            ...
            if exists:
                ...
                if check_filename in str(matched):
                    print(
                        f"SKIPPING: CID item record exists and file found in autoingest: {check_filename}"
                    )
                    logger.warning(
            ...
            if os.path.isfile(of):
                logger.warning(
                    "%s\tDestination file already exists for file: %s", filepath, of
                )
                if segments:
                    logger.warning(
            ...
            print(
                f"Launching document_item.new({object_number}, {relative_segments}, {file_duration}, {extension}, note={note})"
            )
            ...
            if c.partwhole[1] == 1:
                ...
                try:
                    new_object = document_item.new_or_existing(
                        object_number,
                        relative_segments,
                        file_duration,
                        extension,
