...
def log_print(data: str) -> None:
    ...
    """
    with open(
        os.path.join(CODE_PATH, "splitting_scripts/temp_logs/ofcom_item_records.log"),
        "a",
    ) as file:
...
def new_or_existing_no_segments(
    source_object_number: str, extension: str, note=None
) -> Optional[str]:
    """
    Create a new item record for multi-reeler if one doesn't already exist,
    otherwise return the ID of the existing record
...
def new_or_existing_no_segments_mopup(
    source_object_number: str, extension: str, grouping, note=None
) -> Optional[str]:
    """Create a new item record for multi-reeler if one doesn't already exist,
    otherwise return the ID of the existing record"""
    ...
    if hits is None:
        ...
        destination_object = new_no_segments_mopup(
            source_object_number, extension, grouping, note
        )
        return destination_object
...
def already_exists(
    source_object_number: str,
) -> tuple[int, list[dict[str, Optional[Any]]]]:
    """
...
def already_exists_grouping(
    source_object_number: str, grouping_lref: str
) -> tuple[int, list[dict[str, Optional[Any]]]]:
    """
    Has an F47 record already been created for source?
    """
...
def new_no_segments_mopup(
    source_object_number: str, extension: str, grouping: str, note=None
) -> Optional[str]:
    """
