...
def test_check(mocker):
    expected_output = {
        "adlibJSON": {
            "version": [{"spans": [{"text": "AxiellWebApi-Git, Version=3.9.1.3853"}]}]
        }
    }
    ...
    assert (
        result["adlibJSON"]["version"][0]["spans"][0]["text"]
        == "AxiellWebApi-Git, Version=3.9.1.3853"
    )
                ...
                "record": [
                    {
                        ...
                        "grouping": [
                            {
                                "spans": [
                                    {
                                        "text": "Digital Acquisition: Off-Air TV Recording: Automated"
                                    }
                                ]
                            }
                        ],
                        "input.date": [{"spans": [{"text": "2025-10-01"}]}],
                        "input.name": [{"spans": [{"text": "datadigipres"}]}],
                        "input.notes": [
                            {
                                "spans": [
                                    {
                                        "text": "STORA off-air television capture - automated bulk documentation"
                                    }
                                ]
                            }
                        ],
                        ...
                        "object_number": [{"spans": [{"text": "N-10768675"}]}],
                        "priref": [{"spans": [{"text": "159193158"}]}],
                        "record_type": [
                            {
                                "@lang": "neutral",
                                "value": [
...
@pytest.mark.parametrize("expected_method_input", [("updaterecord"), ("insertrecord")])
def test_post(mocker, expected_method_input):
    ...
    result = adlib_sess.post(
        "https://fake-api.com",
        "<xml>payload</xml>",
        "test_db",
        expected_method_input,
        mock_session,
    )
...
def test_create_record_data(monkeypatch, mocker):
    ...
    def fake_get_grouped_items(api, database, mock_session):
        return {
            "cast": [
                "cast.credit_credited_name",
                "cast.activity.sequence",
                "cast.name",
                "cast.credit_type",
                "cast.credit_on_screen",
                "cast.section",
                "cast.cagroup",
                "cast.sequence",
                "cast.name.lref",
            ...
            "credits": [
                ...
                "credit.type",
                "credit.sequence",
                "credit.activity.sequence",
                "credit.cagroup",
                "credit.on_screen",
                "credit.section",
            ],
            "Production": ["creator.lref", "creator.role", "creator"],
            "utb": ["utb.fieldname", "utb.content"],
            "Title_date": ["title_date_start", "title_date.type"],
            "Production_date": ["production.date.end", "production.date.start"],
            "Description": [
                "description",
                "description.type.lref",
                "description.type",
                "description.date",
            ],
            "Content_person": ["content.person.name.lref", "content.person.name"],
            "Content_subject": ["content.subject.lref", "content.subject"],
            ...
            "Part_of": ["part_of_reference.lref", "part_of_reference"],
            "Parts": ["parts_reference"],
            "Inscription": ["inscription.language"],
            "Related_object": ["related_object.reference.lref", "related_object.title"],
            "Label": ["label.source", "label.text", "label.type"],
            "ACCESS_RIGHTS": ["record_access.rights", "record_access.user"],
            "Content_genre": ["content.genre.lref", "content.genre"],
            "Edit": ["edit.date", "edit.time"],
            "Notes": ["notes"],
        }

    monkeypatch.setattr("adlib_v3_sess.get_grouped_items", fake_get_grouped_items)

    record_data_xml = adlib_sess.create_record_data(
        api, database, mock_session, "", record_data
    )

    assert isinstance(record_data_xml, str)
    assert (
        record_data_xml
        == "<adlibXML><recordList><record><priref>0</priref><user>user1</user><input.date>1999-01-01</input.date><input.time>00:00:00</input.time><input.notes>nooob</input.notes><priref>12345</priref><reference_number>N_1234657_01of01.mov</reference_number><imagen.media.original_filename>N_1234657_01of01.mov</imagen.media.original_filename><object.object_number>N-1234657</object.object_number></record></recordList></adlibXML>"
    )
...
"""
    ...
    result = adlib_sess.add_quality_comments(
        "https://fake-api", "12345", "moooooo", mock_session
    )
