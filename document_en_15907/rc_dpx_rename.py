...
def cid_list_retrieve(database: str, search: str) -> list[str]:
    ...
    hits, records = adlib.retrieve_record(
        CID_API, database, search, "0", ["priref", "object_number", "file_type"]
    )
    if hits is None:
        raise Exception(
            f"CID API could not be reached with {database} search:\n{search}"
        )
    if not records:
...
def main():
    ...
    if not match:
        ...
        LOGGER.warning(
            "Folder name error - part whole not formatted correctly: %s", dpx
        )
    ...
    if source_ob_num is None:
        local_logger(
            f"WARNING! DPX number {dpx} not formed correctly and failed conversion to object number"
        )
        LOGGER.warning(
            "Folder name error: Object number creation failed from DPX folder: %s", dpx
        )
        fail_move(fullpath, dpx)
        sys.exit()
    ...
    if len(derived_items) == 0:
        LOGGER.warning(
            "No derived items found for Source object number: %s", source_ob_num
        )
        LOGGER.info(
            "Moving DPX sequence %s to 'error_renumbering' folder for CID data review",
            dpx,
        )
        local_logger(
            f"WARNING! Failed to find derived items for {source_ob_num} - moving {dpx} to 'error_renumbering' folder"
        )
        fail_move(fullpath, dpx)
        sys.exit()
        ...
        LOGGER.info(
            "RAW and Graded DPX object numbers found (pre-validation): RAW %s GRADED %s",
            raw_ob_num,
            graded_ob_num,
        )
        local_logger(
            f"RAW {raw_ob_num} and Graded {graded_ob_num} DPX item numbers found"
        )
        ...
        if (len(derived_list)) == 1:
            ...
            print(
                f"RAW object number: {raw_ob_num}  Graded DPX number: {graded_ob_num}"
            )
            LOGGER.info(
                "RAW and Graded DPX object numbers found (pre-validation): RAW %s GRADED %s",
                raw_ob_num,
            ...
            LOGGER.warning(
                "More than one derived item has DPX file_type. Cannot identify correct RAW object number - manual help needed"
            )
            local_logger(
                "WARNING! Source has more than one DPX derived item. Cannot isolate new number"
            )
            local_logger(
                f"Please attend to this DPX sequence {dpx} - Manual renaming may be necessary"
            )
    ...
    if graded_priref:
        ...
        if "Graded DPX" not in str(gdata):
            ...
            raw_ob_num = None
            LOGGER.warning(
                "FAIL: Graded object number does not have 'Graded DPX' in production.notes"
            )
            local_logger(
                "FAILED: Graded Item record does not have 'Graded DPX' in production.notes"
            )
        else:
            print(gdata)
            LOGGER.info("PASS: Graded Item record has 'Graded DPX' in production.notes")
            local_logger(
                "PASS: Graded Item record has 'Graded DPX' in production.notes"
            )
    else:
    ...
    if raw and raw_ob_num:
        ...
        LOGGER.warning(
            "DPX source object number not matched to any Item record's source_item"
        )
        LOGGER.info(
            "Moving DPX sequence %s to 'error_renumbering' folder for CID data review",
            dpx,
    ...
    if success:
        ...
        LOGGER.warning(
            "WARNING! Folder %s was not renamed after RAW DPX Item object number %s",
            source_ob_num,
            dpx_rename,
        )
        local_logger(
            f"WARNING! Folder {source_ob_num} was not renamed after RAW DPX Item object number {dpx_rename}"
        )
        sys.exit()
    ...
    if move_path is None:
        ...
        LOGGER.warning(
            "WARNING! Folder contents of %s are not three/four depth. Moving to DPX_FOR_REVIEW",
            success[1],
        )
        local_logger(
            f"WARNING! Folder {success[1]} does not have regular folder contents. Moving to DPX_FOR_REVIEW"
        )
        move_success = move(success[0], "review")
    elif "dpx_to_assess_fourdepth" in move_path:
        LOGGER.info(
            "%s folder has four folder structure and will be moved to DPX_TO_ASSESS_FOURDEPTH",
            success[1],
        )
        local_logger(
            f"{success[1]} has folder depth four and will be moved to DPX_TO_ASSESS_FOURDEPTH"
        )
        move_success = move(success[0], "encoding4")
    else:
        LOGGER.info(
            "%s folder has folder depth of three and will be moved to DPX_TO_ASSESS",
            success[1],
        )
        local_logger(
            f"{success[1]} has folder depth of three and will be moved to DPX_TO_ASSESS"
        )
    ...
    local_logger(
        "=============== Renaming and movement of DPX sequence completed =============="
    )
...
def fail_move(fullpath: str, dpx: str) -> bool:
    ...
    if move_success:
        ...
        local_logger(
            f"WARNING! Failed to move {dpx} to 'error_numbering' folder. Please attend to manually"
        )
        local_logger(
            "=============== Renaming and movement of DPX sequence completed =============="
        )
...
def rename(filepath: str, ob_num: str) -> tuple[str, str]:
    ...
    try:
        ...
        LOGGER.warning(
            "rename(): There was an error renaming %s to %s", filename, new_filename
        )
...
def move(filepath: str, arg: str) -> bool:
    ...
    if arg == "error":
        ...
        if "/raw" in str(path):
            ...
            move_path = os.path.join(
                RENUMBER_PATH, "error_renumbering/graded/", filename
            )
    if arg == "encoding3":
        path, filename = os.path.split(filepath)
    ...
    try:
        ...
        LOGGER.warning(
            "Error trying to move file %s to %s.\nError: %s", filename, move_path, err
        )
        return False
