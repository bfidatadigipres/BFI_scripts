...
# =========================================
# Launcher script for stora_housekeeping.py
# =========================================
...
function control {
    boole=$(cat "${CODE}stora_control.json" | grep "stora_qnap04" | awk -F': ' '{print $2}')
    if [ "$boole" = false, ] ; then
...
if [ "$year" = "$last_year" ]; then
    ...
    find "${STORA_PATH}${year}" -maxdepth 4 -mindepth 4 -type d > "${code_pth}dump_text.txt"
else
    echo "Years differ, checking ${year} and ${last_year} paths for folders"
    # Generate txt file containing all folders 5 directories deep only:
    find "${STORA_PATH}${year}" -maxdepth 4 -mindepth 4 -type d > "${code_pth}dump_text.txt"
    find "${STORA_PATH}${last_year}" -maxdepth 4 -mindepth 4 -type d >> "${code_pth}dump_text.txt"
fi

if [ -s "${code_pth}dump_text.txt" ]
  then
    ...
    "$PYENV_DDP" "${CODE_BFI}document_en_15907/stora_housekeeping.py"
