...
def framemd5_manifest(cmd: list[str]) -> bytes:
    ...
    try:
        ...
        raise RuntimeError(
            f"Command {err.cmd} returned with error (code {err.returncode}): {err.output}"
        ) from err
...
def construct_command(
    in_file: str, start: str, out_file: str, end=None, ffmpeg=None
) -> list[str]:
...
def main():
    ...
    parser.add_argument(
        "--start", type=str, required=True, help="Extract from [hh:mm:ss]"
    )
    parser.add_argument(
        "--end", help="Extract until position given as [hh:mm:ss] or [s]"
    )
    parser.add_argument(
        "--output", type=str, required=True, help="Create clip with filename"
    )
