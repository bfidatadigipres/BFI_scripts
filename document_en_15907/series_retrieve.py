...
def retrieve(fullpath: str) -> None:
    ...
    LOGGER.info(
        "========== Fetch series_retrieve.py script STARTED ==============================================="
    )
            ...
            for key, value in subdct.items():
                ...
                if type == "series":
                    try:
                        ...
                        pass
    LOGGER.info(
        "========== Fetch series_retrieve.py script ENDED ================================================"
    )


def get_asset(series_id: str, title: str) -> None:
    series_url = os.path.join(URL, series_id)
    dct = requests.request(
        "GET", series_url, headers=HEADERS, params=QUERY, timeout=1200
    )
    try:
        ...
        LOGGER.warning(
            ("** WARNING: Exporting series data has failed to output to %s", fname), err
        )
    else:
        LOGGER.info(
