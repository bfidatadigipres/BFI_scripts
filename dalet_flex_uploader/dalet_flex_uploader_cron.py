    ...
    if os.path.exists(sys.argv[1]):
        ...
        if file_name.endswith((".mov", ".mkv", ".MOV", ".MKV")):
            ...
            if success is True:
                ...
                log_list.append(f"Upload to AWS took {round(upload_time, 2)} in minutes (or parts of)")
                log_list.append(f"Moving to completed folder {move_path}")
                try:
                    shutil.move(fullpath, move_path)
                    if not os.path.exists(move_path):
                        log_list.append("WARNING: File did not arrive in upload_complete/ folder!")
                        raise Exception
                    else:
                        log_list.append(f"MOVE SUCCESS: {file_name} moved to {move_path}")
                except Exception as error:
                    log_list.append(f"MOVE FAIL: {fullpath} did not move to {PA_SUCCESS}\n{error}")
                    raise error

            else:
                log_list.append(f"Copy to AWS bucket failed: {success}")
                log_list.append(f"Failed upload to AWS took {upload_time} minutes")
                log_list.append(f"Five retry attempts failed, please check file {file_name}")
                log_list.append(f"Moving to errors folder {os.path.join(PA_ERRORS, file_name)}")
                try:
                    shutil.move(fullpath, os.path.join(PA_ERRORS, file_name))
                    log_list.append(f"MOVE SUCCESS: {fullpath} moved to {PA_ERRORS}")
                except Exception as error:
                    log_list.append(f"MOVE FAIL: {fullpath} did not move to {PA_ERRORS}\n{error}")
                    raise error
            ...
            if len(match_list) == 2:
                if match_list[0] != file_name:
                    ...
                    log_list.append(f"Error locating matching file to {fullpath}. Exiting.")
                    dump_to_log(log_list)
                    sys.exit()
            ...
            log_list.append(f"--------- FOUND {file_name} {video_file} - {now[:19]} ---------")
            if check_if_uploaded(video_file) is True:
                log_list.append("** Filename found in uploaded folder - skipping!")
                sys.exit()

            log_list.append(f"New split files found in H22 PlayerArchive folder:\n{fullpath}\n{video_fullpath}")
            ...
            if success is True:
                ...
                try:
                    ...
                    log_list.append(f"MOVE FAIL: {video_fullpath} did not move to {PA_SUCCESS}\n{error}")
                    raise error

            else:
                log_list.append(f"Copy to AWS bucket failed: {success}")
                log_list.append(f"Failed upload to AWS took {upload_time} minutes")
                log_list.append(f"Five retry attempts failed, please check file {file_name} {video_file}")
                log_list.append(f"Moving to errors folder {os.path.join(PA_ERRORS, file_name)}")
                try:
                    shutil.move(fullpath, os.path.join(PA_ERRORS, file_name))
                    log_list.append(f"MOVE SUCCESS: {fullpath} moved to {PA_ERRORS}")
                ...
                try:
                    ...
                    log_list.append(f"MOVE SUCCESS: {video_fullpath} moved to {PA_ERRORS}")
                except Exception as error:
                    log_list.append(f"MOVE FAIL: {video_fullpath} did not move to {PA_ERRORS}\n{error}")
                    raise error

            now = str(datetime.datetime.now())
            log_list.append(f"--------- COMPLETE {file_name} {video_file} - {now[0:19]} ---------")
...
def check_if_uploaded(fname):
    ...
    '''
    uploaded_files = [x for x in os.listdir(PA_SUCCESS) if os.path.isfile(os.path.join(PA_SUCCESS, x))]
