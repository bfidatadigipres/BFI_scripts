...
LOG = os.path.join(
    os.environ["LOG_PATH"], "special_collections_rename_born_digital.log"
)
...
def cid_retrieve(
    ...
    if "priref" in str(rec):
        ...
        record = adlib.retrieve_record(
            CID_API, "collect", search, "0", session, fields
        )[1]
...
def main():
    ...
    for work in work_directories:
        ...
        for image in sorted_images:
            ...
            LOGGER.info(
                "* New Item record created for image %s Born Digital %s",
                image,
                digi_priref,
            )
            ...
            if len(digi_priref) == 0:
                LOGGER.warning(
                    "Missing Born Digital priref following record creation for %s",
                    image,
                )
                LOGGER.warning(
            ...
            if len(digi_obj) > 0:
                ...
                LOGGER.warning(
                    "Object number was not returned following creation of CID Item record for born digital."
                )
                continue
    ...
    LOGGER.info(
        "=========== Special Collections rename - Born Digital END =============="
    )


#
def build_defaults(
    work_data: Optional[tuple[str, str, str, str, str, str]], ipath: str, image: str
