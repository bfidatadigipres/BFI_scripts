    ...
    for file in wav_files:
        ...
        if file.endswith(("01of01.wav", "01of01.WAV")):
            ...
            if re.match(".+01of*", file):
                ...
                if not range_list:
                    local_log(
                        f"Skipping: Range extraction failed for multipart filename {file}"
                    )
                    LOGGER.warning(
                        "Range could not be extracted from multipart filename: %s", file
                    )
                    continue
                local_log(
        ...
        if singleton and "PASS!" in str(success):
            ...
            if len(source_ob_num) > 0:
                ...
                if cid_data[11] != source_ob_num:
                    LOGGER.warning(
                        "Source object number <%s> is not a match to filename converted object_number <%s>",
                        cid_data[11],
                        source_ob_num,
                    )
                    continue
            ...
            if not "Magnetic (sound)" in str(cid_data[15]):
                LOGGER.info(
                    "Source item record is not Magnetic (sound) in sound_system_item: %s",
                    str(cid_data[15]),
                )
            ...
            if len(cid_data[9]) > 0:
                ...
                local_log(
                    f"Creating new CID item record using parent title: {cid_data[9]}"
                )
                wav_data = create_wav_record(
                    cid_data[0], cid_data[9], cid_data[10], cid_data[14], cid_data[11]
                )
                ...
                wav_data = create_wav_record(
                    cid_data[0], cid_data[1], cid_data[2], cid_data[6], cid_data[11]
                )
            else:
                local_log(
        ...
        if multipart and "PASS!" in success_check_string:
            ...
            for (
                ...
                if "FAIL!" in str(val):
                    ...
                    local_log(
                        f"Please resupply and conformance check this file manually. Object number: {wav_data[1]}"
                    )
...
def create_wav_record(
    ...
    try:
        ...
        LOGGER.exception(
            "Unable to retrieve priref and object number from new item record!"
        )
        return None
