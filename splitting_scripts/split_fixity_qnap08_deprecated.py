...
def main():
    ...
    for filepath in files:
        ...
        logger.info(
            "* Carrier directories created\nQNAP-01 %s / QNAP-08 %s",
            qnap_01_carrier_directory,
            qnap_08_carrier_directory,
        )
        ...
        for item in c.items:
            ...
            if exists:
                ...
                logger.info(
                    "%s\tChecking if file has already persisted to DPI, or is in autoingest paths",
                    filepath,
                )
                ...
                print(
                    f"Checking if {firstpart_check} or {check_filename} persisted to DPI or are in autoingest"
                )
            ...
            logger.info(
                "%s\tNo derived CID item record already exists for object number %s.",
                filepath,
                object_number,
            )
            # If destination file already exists, move on
            of_01 = os.path.join(
                qnap_01_carrier_directory, f"{object_number}.{extension}"
            )
            of_08 = os.path.join(
                qnap_08_carrier_directory, f"{object_number}.{extension}"
            )
            logger.info(
                "%s\t* Destinations created for QNAP-01 and QNAP-08: \n%s\t%s",
                filepath,
                of_01,
                of_08,
            )
            if os.path.isfile(of_01):
                ...
                logger.warning(
                    "%s\tDestination file already exists for file: %s", filepath, of_08
                )
                continue
            ...
            if segments:
                ...
                if pos == 0:
                    ...
                    if a < in_:
                        logger.warning(
                            "%s\t%s\tInvalid video_part data: item begins before end of preceding item\t{}",
                            filepath,
                            id_,
                            in_,
                        )
            ...
            if not make_segment:
                ...
                try:
                    ...
                    logger.info(
                        "%s\tMoved single-item file\t%s -> %s", filepath, f, of_08
                    )
                ...
                if not fixity:
                    # Log FFmpeg error
                    ffmpeg_message = (
                        f"{filepath}\tFFmpeg failed to create Matroska file\t{of_01}"
                    )
                    ...
                    try:
                        os.remove(of_01)
                        logger.info(
                            "%s\tDeleted invalid Matroska file\t%s", filepath, of_01
                        )
                    except Exception as err:
                        logger.warning(
            ...
            if os.path.isfile(of_01):
                ...
                if os.path.isfile(of_08):
                    print(
                        f"End fixity segmentation at {datetime.datetime.now().ctime()}"
                    )
                    logger.info(
                        "%s\tEnd fixity segmentation at %s",
                        filepath,
                    ...
                    print(
                        f"* Split file does not exist, do not proceed with CID item record creation: {of_01} or {of_08}"
                    )
                    logger.info(
                        "%s\tSplit file does not exist. Moving to next item.\n%s\t%s",
                        filepath,
                        of_01,
                        of_08,
                    )
            ...
            if c.partwhole[1] == 1:
                ...
                logger.info(
                    "%s\tCreated new CID Item for Matroska file: %s",
                    filepath,
                    object_number,
                )
            ...
            # Multi-part
                ...
                # New object successfully retrieved / created
                logger.info(
                    "%s\tCreated / Retrieved CID Item for Matroska file: %s",
                    filepath,
                    object_number,
                )
            ...
            logger.info(
                "%s\tMultipart partwhole retrieved from models: %s",
                filepath,
                c.partwhole,
            )
