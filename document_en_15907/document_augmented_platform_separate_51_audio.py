...
def main() -> None:
    ...
    for key, value in STORAGE.items():
        ...
        for fpath in folder_list:
            ...
            if len(file_list) != 6:
                LOGGER.warning(
                    "Skipping. Incorrect amount of files found in path: %s", fpath
                )
                continue
            LOGGER.info(
                "Files found in target folder %s: %s",
                object_number,
                ", ".join(file_list),
            )
            ...
            for key, value in file_names.items():
                ...
                if not old_fname.endswith((".WAV", ".wav")):
                    LOGGER.warning(
                        "File contained in separate5_1 audio folder that is not WAV: %s",
                        old_fname,
                    )
...
def make_item_record_dict(
    priref: str, record: list[dict[str, Any]]
