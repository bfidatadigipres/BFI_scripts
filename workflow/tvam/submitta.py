...
for TVAM 1 inch Multi Machine Environment in F47

Dependencies:
1. LOGS/tvam_submitta.log
2. tvam/submissions.csv
3. tvam/errors.csv
"""
...
def main():
    ...
    for _ in range(0, batches_per_iteration):
        ...
        # JMW - 10 day deadline? Calculate deadline
        ...
        if not batch.successfully_completed:
            ...
            print(f"Error creating TVAM Workflow job: {batch.priref}")


def write_to_log(message):
    """
    Write to tvam submitta log
    """
    with open(os.path.join(LOGS, "tvam_submitta.log"), "a") as file:
        file.write(message)
        file.close()
