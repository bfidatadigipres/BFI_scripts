...
AMAZON: Final = os.path.join(STORAGE, "AMAZON")
...
def get_folder_match(foldername: str) -> list[str]:
    ...
    """
    folder_list: list[str] = [x for x in os.listdir(AMAZON) if x.startswith(foldername)]
...
def get_cat_data(data=None) -> Optional[dict[str, str]]:
    ...
    val.certification.get("amazon") and c_data.update(
...
def get_json_data(data=None) -> Optional[dict[str, str]]:
    ...
    val.certification.get("amazon") and j_data.update(
        {"cert_amazon": val.certification.get("amazon")}
    )
...
def get_season_data(data=None) -> Optional[dict[str, str]]:
    ...
    val.vod.get("amazon-prime-uk").get("start") and s_data.update(
        {"start_date": val.vod.get("amazon-prime-uk").get("start")[:10]}
    )
...
def cid_check_works(
    ...
    try:
        ...
        print(
            "cid_check_works(): Record not found. Series hit count and series priref will return empty strings"
        )
        return None
    if hits is None:
    ...
    for num in range(0, hits):
        ...
        if hits is None:
            LOGGER.exception(
                '"CID API was unreachable for Works search: %s', type_query
            )
            raise Exception(f"CID API was unreachable for Works search: {type_query}")
        try:
            alt_num_type = adlib.retrieve_field_name(
                type_record[0]["Alternative_number"][0], "alternative_number.type"
            )[0]
    ...
    for num in range(0, csv_range):
        ...
        if "film" in level.lower() or "programme" in level.lower():
            ...
            if "contributors" in data_dct and len(data_dct["contributors"]) >= 1:
                ...
                if success:
                    LOGGER.info(
                        "Contributor data written to Work record: %s", priref_work
                    )
                else:
                    LOGGER.warning(
            ...
            priref_man = create_manifestation(
                priref_work, work_title, work_title_art, data_dct, record, manifestation
            )
            if len(priref_man) == 0:
                LOGGER.warning(
                    "Monograph manifestation record creation failed, skipping all further record creations"
                )
                continue
            ...
            priref_item = create_item(
                priref_man, work_title, work_title_art, data_dct, record, item
            )
            if len(priref_item) == 0:
                LOGGER.warning(
                    "Monograph item record creation failed, skipping all further stages"
                )
                continue
...
def genre_retrieval_term(
    category_code: str, description: str, title: str
) -> tuple[str]:
    """
...
def build_defaults(data: dict[str, str]) -> list[dict[str, str]]:
    ...
    manifestation: list[dict[str, str]] = [
    ...
    if "d_medium" in series_dct:
        label_fields.append(
            [
                ...
                {"label.text": series_dct["d_medium"]},
                {"label.source": "EBS augmented EPG supply"},
                {"label.date": str(datetime.datetime.now())[:10]},
            ]
        )
...
def create_work(
    ...
    work_title: str,
    work_title_art: str,
    work_dict: list[dict[str, str]],
    record_def: dict[str, str],
    work_def: list[dict[str, str]],
    work_restricted: list[dict[str, str]],
) -> str:
    """
...
def create_manifestation(
    ...
    try:
        ...
        print(
            f"* Unable to create Manifestation record for <{work_dict['title']}> {err}"
        )
        LOGGER.critical(
            "** Unable to create Manifestation record for <%s>", work_dict["title"]
        )
        return None

    broadcast_addition = [{"broadcast_company.lref": "999823516"}]  # Amazon Prime Video
...
def append_url_data(work_priref: str, man_priref: str, data=None):
    ...
    LOGGER.info(
        "append_url_data(): Watch URL data updated to Manifestation: %s", man_priref
    )
    ...
    try:
        ...
        if item_rec:
            try:
                ...
                item_object_number = adlib.retrieve_field_name(
                    item_rec, "object_number"
                )[0]
