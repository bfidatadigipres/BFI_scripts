...
hdlr = logging.FileHandler(
    os.path.join(os.environ["LOG_PATH"], "fetch_series_stora_augmented.log")
)
...
def main() -> None:
    ...
    check_control()
    logger.info(
        "========== Series Cache fetch metadata script STARTED ==============================================="
    )
                        ...
                        for key, value in subdct.items():
                            ...
                            if types == "series":
                                try:
                                    ...
                                    logger.info(
                                        "Getting asset details from series_id: %s - %s",
                                        series_id,
                                        title,
                                    )
    ...
    logger.info(
        "========== Fetch series augmented metadata script ENDED ================================================"
    )
...
def get_asset(series_id: str, title: str) -> None:
    ...
    try:
        ...
        logger.warning(
            "** WARNING: Exporting series data has failed to output to %s\n%s",
            fname,
            err,
        )
    else:
        logger.info(
            "Export of asset series metadata has been successful. Saving to top folder %s",
            STORAGE_PATH,
        )
